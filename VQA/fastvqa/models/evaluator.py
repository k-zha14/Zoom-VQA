...
class DiViDeAddEvaluator(nn.Module):
    def __init__(
        ...
        for key, hypers in backbone.items():
            print('Backbone type:', backbone_size)
            if key not in self.backbone_preserve_keys:
                continue
            
            # determine the whole network strcuture
            if backbone_size=="divided":
                t_backbone_size = hypers["type"]
            else:
                t_backbone_size = backbone_size
            
            # determine the main backbone 
            if t_backbone_size == 'swin_tiny_grpb':
                b = VideoBackbone(drop_path_rate=droppath_ratio, patch_size=patch_size)    # window_size=(8, 7, 7), frad_iases=(True, True, True, True)
            
            elif t_backbone_size == 'swin_small_grpb':
                b = VideoBackbone(depths=[2, 2, 18, 2], drop_path_rate=droppath_ratio, patch_size=patch_size, 
                                  frag_biases=[True, True, True, False],)
            elif t_backbone_size == 'swin_tiny_grpb_m':
                # reproduce fast-vqa-m
                b = VideoBackbone(window_size=(4,4,4), frag_biases=[0,0,0,0])
            elif t_backbone_size == 'swin_tiny':
                b = swin_3d_tiny()
    ...
    def forward(self, vclips, inference=True, return_pooled_feats=False, reduce_scores=True, pooled=False, **kwargs):
        if inference:
            ...
            feats = {}
            for key in vclips:
                # extract backbone features, [B, C, T, H, W] -> [16, 768, 16, 7, 7]
                feat = getattr(self, key.split("_")[0]+"_backbone")(vclips[key], multi=self.multi, layer=self.layer, **kwargs)
                # print('features of backbone:', feat.shape)
                # sys.exit(0)

                if hasattr(self, key.split("_")[0]+"_head"):
                    scores += [getattr(self, key.split("_")[0]+"_head")(feat)]
                else:
                    # FastVQA run in this way, output [B, C, T, H, W] -> [16, 1, 16, 7, 7]
                    scores += [getattr(self, "vqa_head")(feat)]
                    # print('scores:', scores[0].shape, len(vclips))
                    # sys.exit(0)
